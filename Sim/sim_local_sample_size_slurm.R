# Simulation: local sample size under slurm jobs in HPC 
# Section 4.3 Effect of Local Sample Size
# Effect of local sample size for varying local sample sizes and the dimensions $(n, p)$ in $\{(100, 100), (200, 100), (200, 200)\}$.
...
library(dplyr)
n_values <- c(100, 200, 200)
p_values <- c(100, 100, 200)
...
params_slurm <- expand.grid(
  ...
  dplyr::select(batch, n, p, rho)
# table(params_slurm[,c("p", "n")])
...
result <- result %>%
  dplyr::select(dplyr::starts_with("RMSE"), dplyr::starts_with("F1_"),
