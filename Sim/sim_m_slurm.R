# Simulation: the number of machines under slurm jobs in HPC 
# Section 4.4 Effect of Topology  
# Number of machines/nodes analysis that fix the total sample size to \(N = 4000\) and vary the number of nodes \(m \in \{5, 10, 20\}\). 
...
library(dplyr)
m_values <- c(5, 10, 20)
n_values <- c(840, 420, 210)
mn_df <- data.frame(m = m_values, n = n_values)
params_slurm <- expand.grid(
  batch = 1:100,
  m_index = 1:length(m_values),
  ...
  dplyr::select(batch, m, n, rho, pc, C_h)
...
sjob <- slurm_apply(simulation, params_slurm, nodes = 100,
                    ...
                    global_objects = ls(),
                    slurm_options = list(time = "3:00:00", `mem-per-cpu` = "500MB"))
...
result <- result %>%
  dplyr::select(starts_with("RMSE"), starts_with("F1_"),
         setdiff(names(params_slurm), c("batch", "n", "C_h")))


out_table <- aggregate(as.formula(paste0(". ~ ", paste(setdiff(names(params_slurm), c("batch", "n", "C_h")), collapse =
  "+"))),
                       result, mean)
out_table
