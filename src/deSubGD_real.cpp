...
//' @export
// [[Rcpp::export]]
Rcpp::List deSubGD_single_svm_real(arma::mat &X,
                               arma::vec &y,
                               arma::mat &adjacency_matrix,
                               arma::mat &B_init,
                               arma::vec &divisions, // divisions of real data
                               int T = 20,
   ...
   for (int j = 0; j < m; j++) {
     sample_indices[j] = getSampleIndices(divisions, j + 1);  // j+1 division
   }
   ...
   for(t = 0; t < T; t++) {
     ...
     for(j = 0; j < m; j++) {
       ...
       int n_j = idx.n_elem;  // sample size at jth node

       g_mat.col(j).zeros();  // initialize the subgradient

       for (int i = 0; i < n_j; i++) {
         double margin = y(idx(i)) * arma::dot(X.row(idx(i)), B.col(j)); // y_i * (X_i * B_j)
         if (margin < 1) {
           g_mat.col(j) -= y(idx(i)) * X.row(idx(i)).t(); // update gradient
         }
       }
     ...
     Phi = B - eta * g_mat; // update estimation
...
//' @export
// [[Rcpp::export]]
Rcpp::List deSubGD_svm_real(arma::mat &X,
                        arma::vec &y,
                        arma::mat &adjacency_matrix,
                        arma::mat &B_init,
                        arma::vec &divisions,
   ...
   for (int ilambda = 0; ilambda < lambda_array.n_elem; ilambda++) {
     ...
     for (int j = 0; j < m; j++) {
       ...
       arma::vec result = arma::ones<arma::vec>(n_j) - y(idx) % (X.rows(idx) * B.col(j));
       bic_array(ilambda) += arma::accu(pmax_arma(result, 0.0));
       shat = std::max(shat, int(arma::accu(B.col(j).subvec(1, B.n_rows - 1) != 0)));
     }

     bic_array(ilambda) = bic_array(ilambda) + std::log(arma::sum(N)) * std::sqrt(std::log(arma::sum(N))) * std::max(shat, 1);
